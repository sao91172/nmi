...
let names = document.querySelector('#aname');
let pics = document.querySelector('#image');

for(i=0; i<nameArray.length; i++){
    names.innerHTML += "<option value="+nameArray[i]+">"+nameArray[i]+ "</option>"
}
...
function quote(){
    ...
    if(classObject.hasOwnProperty(sName)){
     document.querySelector('#aboutme').innerHTML = classObject[sName];
     document.querySelector('#image').innerHTML = "<div id=image>"+ "<img src=pix/" + sName +".jpg></div>";
    } 
    else  {
        document.querySelector('#aboutme').innerHTML = sName+ " is not present in the class";
    }
